...
main:{strcpy(debug,"rule main");} datatype IDENTIFICATEUR { ajouter('F'); }
;
externs: externs externe
|{strcpy(debug,"rule externes");}externe
;
externe:{strcpy(debug,"RExterne");} EXTERN datatype IDENTIFICATEUR { ajouter('F'); } '(' liste_parms ')' ';'
;

liste_parms	:{strcpy(debug,"Rlist params 1");} liste_parms ',' parm
	|{strcpy(debug,"Rparam 1");}	parm
	| 
;
parm	:	
...
liste_instructions :	instruction liste_instructions   {$$.nd=faire_noeud($1.nd,$2.nd,"instructions");}
	...
	|
;
instruction: FOR { ajouter('K'); is_for = 1; } '(' instruction1 ';' condition ';' instruction1 ')' '{' liste_instructions '}' { 
...
instruction1: datatype IDENTIFICATEUR { ajouter('V'); } init { 
...
void yyerror(const char *msg) {
    ...
    fprintf(stderr, "L'entree etait: %.*s\n", yyleng, yytext);
