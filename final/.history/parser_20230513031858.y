...
$1.nd_dot = faire_noeud_lcrs(NULL, NULL, "main","ellipse","dotted");
$$.nd_dot = faire_noeud_lcrs(NULL, NULL, "programme","ellipse","dotted");
head_dot=$$.nd_dot;
} 
| externs programme {
	$$.nd=$2.nd;
...
struct noeud_lcrs* faire_noeud_lcrs(struct noeud_lcrs *left_child, struct noeud_lcrs *right_sibling, char *label,char *style,char *shape) {	
	...
	char *newlabel1 = (char *)malloc(strlen(label)+1);
	strcpy(label1, label);
	...
	newnode->style=style1;
	newnode->shape=newsshape1;
	return(newnode);
}
...
void affichage_prifixe_de_larbre_syntaxique_dot_lcrs(FILE* f,struct noeud_lcrs *tree,int* j) {
	...
	if(tree->style)fprintf(f,"shape=\"%s\" ", tree->style);
	fprintf(f,"]");
	tree->inode=*j;
	*j=*j+1;
	if (tree->left_child) {
