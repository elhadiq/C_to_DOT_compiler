...
| IDENTIFICATEUR { verefier_declaration($1.nom); } '=' expression {
	...
	sprintf(buff2,"label=%s",$1.nom);
	$$.nd_dot=faire_noeud_lcrs(faire_noeud_lcrs(NULL,$4.nd_dot,buff2),NULL,"label=\":=\"");
}
...
appel	:	
		IDENTIFICATEUR '(' liste_expressions ')' ';'
		{
			...
			sprintf(buff2,"label=%s shape=septagon",$1.nom);
				$$.nd_dot=faire_noeud_lcrs($3.nd_dot,NULL,buff2);
		}
;
variable	:	
		IDENTIFICATEUR  
...
expression	:	
	...
	sprintf(buff2,"label=\"%s\"",$2.nom);
	$$.nd_dot=faire_noeud_lcrs($1.nd_dot,NULL,buff2);
