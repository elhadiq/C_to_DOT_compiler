...
int main() {
	system("mkdir -p temp");
	system(" rm -r y.tab.c y.tab.h y.output lex.yy.c output.pdf output.dot mycompiler");
    yyparse();
    printf("\n\n");
	...
	FILE *fp;
	fp = fopen("temp/ArbreSyntaxique.dot", "w"); // create or open the file for writing
	...
	FILE *fp_dot;
	fp_dot = fopen("temp/output.dot", "w"); // create or open the file for writing
