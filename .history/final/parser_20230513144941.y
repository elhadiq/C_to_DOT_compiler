...
main:{sprintf(debug[i_deb++],"1");} datatype IDENTIFICATEUR { ajouter('F'); }
{sprintf(buff,"%s, %s",$3.nom,$2.nom);$$.nd_dot=faire_noeud_lcrs(NULL, NULL, buff,"invtrapezium","");}
;
externs:{sprintf(debug[i_deb++],"2");} externs externe
|{sprintf(debug[i_deb++],"2.1");} externe
;
externe:{sprintf(debug[i_deb++],"2.2");} EXTERN datatype IDENTIFICATEUR { ajouter('F'); } '(' liste_parms ')' ';'
;

liste_parms	:{sprintf(debug[i_deb++],"2");} liste_parms ',' parm
	|{sprintf(debug[i_deb++],"2.1");}	parm
	| 
;
parm	:	{sprintf(debug[i_deb++],"3");}
		INT { inserer_type(); } IDENTIFICATEUR { ajouter('V'); }
;
liste_declarations	:	{sprintf(debug[i_deb++],"4");}
		liste_declarations declaration 
	|{sprintf(debug[i_deb++],"4.1");}	declaration
;
liste_declarateurs	:{sprintf(debug[i_deb++],"5");} declarateur ':' liste_declarateurs 
	|{sprintf(debug[i_deb++],"5.1");}	declarateur
;
declarateur	:{sprintf(debug[i_deb++],"6");}  IDENTIFICATEUR {ajouter('V');}
	|{sprintf(debug[i_deb++],"6.1");}	declarateur '[' CONSTANTE {ajouter('C');}']'
;
declaration	:	{sprintf(debug[i_deb++],"7");} datatype liste_declarateurs ';'
;
datatype:{sprintf(debug[i_deb++],"8");}  INT { inserer_type(); }
|{sprintf(debug[i_deb++],"2");} VOID { inserer_type(); }
;
...
instruction1:{strcpy(debug[i_deb++],"ID1");} datatype IDENTIFICATEUR {ajouter('V');} ";"
...
void yyerror(const char *msg) {
    ...
    fprintf(stderr, "last rules");
	for(int i=0;i<=i_deb;i++)printf("\n%s",debug[i]);
}
