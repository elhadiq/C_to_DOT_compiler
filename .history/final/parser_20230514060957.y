...
| FOR { ajouter('K'); is_for = 1; } '(' instruction1 ';' condition ';' instruction1 ')' '{' liste_instructions '}' { 
	...
	$$.nd_dot=faire_noeud(NULL,NULL,"label=inst7");
}
| IF { ajouter('K'); is_for = 0; } '(' condition ')' { sprintf(code3v[DOT_index++], "\nLABEL %s:\n", $4.if_corps); } '{' liste_instructions '}' { sprintf(code3v[DOT_index++], "\nLABEL %s:\n", $4.else_corps); } else { 
	...
	$$.nd_dot=faire_noeud(NULL,NULL,"label=inst8");
}
| instruction1 ';' { $$.nd = $1.nd; $$.nd_dot=faire_noeud(NULL,NULL,"label=inst9");}
| appel {$$.nd=$1.nd;$$.nd_dot=faire_noeud(NULL,NULL,"label=inst10");}
|declaration {$$.nd_dot=faire_noeud(NULL,NULL,"label=inst10");}
;
