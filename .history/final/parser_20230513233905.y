%{
	...
	void ajouter_dernier_fils ajouter_dernier_fils(struct noeud_lcrs* pere,struct noeud_lcrs* fils);
	struct dataType {
        char * nom_id;
        char * type_data;
        char * type;
...
%type <nd_obj> externe externs main liste_instructions liste_declarations declaration liste_declarateurs declarateur liste_expressions returne appel datatype instruction1 arithmetic relop programme else instruction binary_op
...
$5.nd_dot = faire_noeud_lcrs(NULL, NULL, "label=bloc");
$$.nd_dot = $1.nd_dot;
$1.nd_dot->left_child=$5.nd_dot;
$5.nd_dot->left_child=faire_noeud_lcrs(NULL, NULL, "label=test");
ajouter_dernier_fils($5.nd_dot,$6.nd_dot);
head_dot=$$.nd_dot;
} 
| externs programme {
	$$.nd=$2.nd;
...
{sprintf(buff,"label=\"%s, %s\" shape=invtrapezium",$2.nom,$1.nom);
...
$$.nd_dot=faire_noeud_lcrs(NULL,NULL,"label=instructions");
}
	| instruction
	|
;
...
expression	:	
	...
	|	expression binary_op expression %prec OP {$$.nd=faire_noeud($1.nd,$3.nd,$2.nom);sprintf(buff,"label=%s",$2.nom);$$.nd_dot=faire_noeud($1.nd_dot,$3.nd_dot,buff);} 
...
valeur: CONSTANTE 
{ strcpy($$.nom, $1.nom); strcpy($$.type, "int"); ajouter('C'); $$.nd = faire_noeud(NULL, NULL, $1.nom); }
