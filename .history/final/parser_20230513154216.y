...
;
externs:{system("clear");printf("am here");} externs externe
|externe
;
...
liste_declarations	:	
	...
	|	{system("clear");printf("declaration 1");}declaration
;
liste_declarateurs	: declarateur ',' liste_declarateurs 
	|{system("clear");printf("declaration 2");}	declarateur
;
declarateur	: {system("clear");printf("declaration 3");} IDENTIFICATEUR {ajouter('V');}
	|	{system("clear");printf("declaration 4");} declarateur '[' CONSTANTE {ajouter('C');}']'
	| IDENTIFICATEUR { ajouter('V'); } init { 
	$1.nd = faire_noeud(NULL, NULL, $2.nom); 
	$$.nd = faire_noeud($2.nd, $4.nd, ":="); 
	sprintf(code3v[DOT_index++], "%s = %s\n", $2.nom, $4.nom);
}
;
declaration	:{system("clear");printf("declaration 5\n");}	
