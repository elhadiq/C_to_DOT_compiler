...
main:{sprintf(debug[i_deb++],"main 1");} datatype IDENTIFICATEUR { ajouter('F'); }
{sprintf(buff,"%s, %s",$3.nom,$2.nom);$$.nd_dot=faire_noeud_lcrs(NULL, NULL, buff,"invtrapezium","");}
;
externs:{sprintf(debug[i_deb++],"e2");} externs externe
| externe
;
...
liste_parms	: liste_parms ',' parm
	|{sprintf(debug[i_deb++],"lp5");}	parm
	| 
;
parm	:{sprintf(debug[i_deb++],"p5");}	
		INT { inserer_type(); } IDENTIFICATEUR { ajouter('V'); }
;
liste_declarations	:{sprintf(debug[i_deb++],"d1");}	
		liste_declarations declaration 
	|	declaration
;
liste_declarateurs	:{sprintf(debug[i_deb++],"d2");} declarateur ':' liste_declarateurs 
	|{sprintf(debug[i_deb++],"d3");}	declarateur
;
declarateur	:{sprintf(debug[i_deb++],"d4");}  IDENTIFICATEUR {ajouter('V');}
	|	{sprintf(debug[i_deb++],"d5");} declarateur '[' CONSTANTE {ajouter('C');}']'
;
declaration	:	{sprintf(debug[i_deb++],"d6");} datatype liste_declarateurs ';'
;
datatype:{sprintf(debug[i_deb++],"dt_int");}  INT { inserer_type(); }
| {sprintf(debug[i_deb++],"dt_void");} VOID { inserer_type(); }
;
...
| {sprintf(debug[i_deb++],"inst 1");} instruction1 ';' { $$.nd = $1.nd; }
